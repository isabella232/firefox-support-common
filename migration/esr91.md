...
* コンテキストメニューの「新しいタブで画像を開く」で、タブがバックグラウンドで開かれるようになりました。 (Firefox 90)
  * 従来の挙動に戻すには `browser.tabs.loadInBackground`=`false` を設定します。
* Windows 10のシングルサインオンをサポートし、Microsoftの対応サイトでWindowsのアカウントでサインオンできるようになりました。 (Firefox 91)
  - 有効化するには、`WindowsSSO`のポリシーを設定します。
  - Firefox 91ベータ版時点ではデフォルトでは有効化されていません。 <!--Bugzilla #1695693 -->
...
* ユーザーに許可を求めず外部アプリケーションを実行できるドメインを指定できるようになりました（Firefox 90)
  * 例えば「 https://www.skype.com`のみ`skype://`を許可を求めずに開けるようにする」といった挙動が実現できます。
