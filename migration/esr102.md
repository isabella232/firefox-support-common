...
* 厳密なトラッキング保護の有効時、およびプライベートブラウジングモードにおいて、SmartBlock 3.0のはたらきにより、Webページ閲覧の互換性が向上しました。[詳細](https://blog.mozilla.org/security/2021/10/05/firefox-93-features-an-improved-smartblock-and-new-referrer-tracking-protections/)（Firefox 93）
...
* DNSの[HTTPS RR（Resource Record）](https://developer.mozilla.org/en-US/docs/Glossary/https_rr)の情報に基づき、HTTPにおける`Alt-Svc`ヘッダと同様の、HTTPSでの通信への自動切り替えを行うようになりました。（Firefox 92）
  - HTTPS RRは、DNSの応答の中に、HTTPSでの通信に必要な各種の情報を事前に含めるための仕様です。この情報を活用することで、ブラウザーは初回の問い合わせの部分的な省略など、より効率の良い通信が可能となります。
...
* Webサイトのオリジンごとに保存可能なデータのクォータが5MBになりました（従来は有効なトップレベルドメインの1段階下のドメイングループ単位でクォータが設定されていました）。（Firefox 92）
  ...
  - クォータの容量は、MCDまたはポリシー設定の `Preferences` で `dom.storage.default_quota` で変更可能ですが、適用単位をオリジンごと以外に変更することはできません。
