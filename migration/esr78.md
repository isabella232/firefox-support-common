...
* プライバシー保護の状況のレポートを、アドレスバーの盾アイコンのクリックで表示できるようになりました。（Firefox 71）  
...
* Webサイトがユーザーの位置情報を参照している事の通知がアドレスバーに表示されるようになりました。（Firefox 70）
...
* ニュースやブログなどを閲覧した時に、通知の許可を求めるポップアップが表示されていたケースについて、ポップアップを表示せずにアドレスバーの中にアイコンが表示されるだけになりました。（Firefox 72）
  ![](esr78/notification-popup.png)
  （この挙動は `dom.webnotifications.requireuserinteraction` で無効化できます。）
* アドレスバーのデザインが変更されました。（Firefox 75）
  ![](esr78/megabar-compare.png)
  これに伴い、アドレスバーにフォーカスしただけで、よく見るサイトの一覧が表示されるようになりました。
  ![](esr78/megabar.png)
* アドレスバーにおいて、[一般的なFirefoxの問題に対する解決策を含む項目が候補に表れるようになりました。](https://support.mozilla.org/ja/kb/search-web-firefox#w_firefox-solution-cualch)現時点では英語版のみ機能します。（Firefox 75）
...
* Linuxでアドレスバーや検索バーをクリックしたときの振る舞いが、他のデスクトップ環境と同じになりました。（シングルクリックで「選択範囲を自動的にクリップボードにコピー」せず全選択になり、ダブルクリックで単語選択、トリプルクリックで選択範囲をクリップボードにコピーする全選択となります。）（Firefox 75）
