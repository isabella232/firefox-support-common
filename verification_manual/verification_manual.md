...
<!--/GROUP-->
{{#Security-15-2}}
1. パネルメニューの「開発ツール」→「WebIDE」でWebIDEを開く。
...
1. 「履歴」メニューを開く。
    - 確認項目
        ...
        1. 「最近閉じたウィンドウ」にウィンドウの項目が1つだけ表示される。(Privacy-10-2)
1. Firefoxを再起動する。
1. 「履歴」メニューを開く。
    - 確認項目
...
1. 前項に引き続き検証するか、または以下の状態を整えておく。
    1. カスタマイズ済みFirefoxのインストールが完了した状態にする。
1. ユーザープロファイルを削除した場合、検証前に1回Firefoxを起動し、ユーザープロファイルが存在する状態にする。
   （初回起動時は必ずプロセス分離が無効化されるため、2回目以降の起動時の状態で検証する。）
...
{{/Ui-6}}
1. 「ツール」メニュー内の「開発ツール」配下の「ウェブコンソール」をクリックする。
{{#Script-1}}
...
1. パネルメニュー内の「開発ツール」をクリックし、サブメニューの「インスペクター」をクリックする。
